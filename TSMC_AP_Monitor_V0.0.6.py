...
def generate_3d_html(data, origin, optimal_trace):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(width=1, color="#a10000"), visible='legendonly',
        hoverinfo="text"
    ), row=2, col=1)
    ...
    fig.update_yaxes(title="ALTITUDE (M)", range=[0, 150], row=2, col=1, gridcolor='#222', title_font=dict(size=14, family="Consolas", color="#eee"))
...
<body><div class="master">
    ...
    function toggleAvg() {{
        ...
        // Optimal traces are at the end: 
        // Total traces = 27*2 (members) + 2 (Band) + 2 (Optimal) = 54 + 4 = 58
        // Indices: 54, 55, 56, 57
        const total = gd.data.length;
        Plotly.restyle(gd, {{visible: on}}, [total-4, total-3, total-2, total-1]);
    }}
    ...
    window.addEventListener('message', function(e) {{
        ...
        if(payload.type === 'updateData') {{
            ...
            // Indices: T-4 to T-1
            // 1. Update visual state of button
